...
maeCode <- c("miscarriage", "ectopic_pregnancy", "stillbirth", "preterm_labour", "dysfunctional_labour", "antepartum_haemorrhage",  "gestational_diabetes", "preeclampsia", "eclampsia", "hellp", "postpartum_endometritis", "postpartum_haemorrhage", "maternal_death")
maeClean <- c("Miscarriage", "Ectopic pregnancy", "Stillbirth", "Preterm labour", "Dysfunctional labour", "Antepartum haemorrhage",  "Gestational diabetes", "Pre-eclampsia", "Eclampsia", "HELLP Syndrome", "Postpartum endometritis", "Postpartum haemorrhage", "Maternal death")
aesiCode <- c("pulmonary_embolism", "deep_vein_thrombosis", "bells_palsy", "immune_thrombocytopenia", "anaphylaxis", "myocarditis_or_pericarditis", "ischaemic_stroke", "haemorrhagic_stroke", "tts",  "guillain_barre_syndrome",  "disseminated_intravascular_coagulation", "transverse_myelitis", "narcolepsy", "myocardial_infarction", "encephalitis")
aesiClean <- c('Pulmonary embolism', 'Deep vein thrombosis', 'Bell’s palsy', 'Immune thrombocytopenia', 'Anaphylaxis', 'Myocarditis or pericarditis', 'Ischaemic stroke', 'Haemorrhagic stroke', 'Thrombosis with thrombocytopenia syndrome', 'Guillain–Barré syndrome', 'Disseminated intravascular coagulation', 'Transverse myelitis', 'Narcolepsy', 'Myocardial infarction', 'Encephalitis')
excludeCPRD <- c("Miscarriage", "Stillbirth")
dbOrder <- c("NLHR", "SCIFI-PEARL", "SIDIAP", "CPRD GOLD")
...
:::{custom-style="Caption"}
Table 3. Pooled Cumulative Incidence overall pregnancy and 95% Prediction Intervals for adverse events across four databases.

:::

```{r}
# prepare results ----
tidySurvival <- data$survival |>
  dplyr::filter(strata_name %in% c("overall", "gestational_trimester", "maternal_age_group", "pregnancy_start_period")) |>
  omopgenerics::filterSettings(result_type == "survival_summary") |> 
  dplyr::filter(estimate_name %in% c("number_records_count", "n_events_count")) |> 
  omopgenerics::tidy() |>
  dplyr::filter(.data$outcome_group %in% c("Maternal Adverse Events", "Adverse Events of Special Interest")) |>
  dplyr::mutate(
    outcome = factor(
      .data$outcome, 
      levels = c(aesiCode, maeCode), 
  ...
  select(!c(
 "target_cohort", "competing_outcome", "socioeconomic_status", "ethnicity", "birth_continent", "nationallity", "variable_name", "variable_level", "outcome_washout", "minimum_survival_days", "follow_up_days", "censor_on_cohort_exit",
 "outcome_date_variable", "analysis_type"
  ))
...
# --> consider databases with zero events
if (runMetaAnalysis) {
  data$meta_cumulative_incidence <- tidySurvival |>
    filter(!is.na(n_events_count), !is.na(number_records_count)) |>
    group_by(
      outcome,
      maternal_age_group,
      pregnancy_start_period,
...
# display results ----
data$meta_cumulative_incidence |>
  dplyr::filter(
    maternal_age_group == "overall",
    pregnancy_start_period == "overall"
  ) |>
  select(!c("maternal_age_group", "pregnancy_start_period")) |>
  pivot_longer(
    cols = c("pooled_estimate", "predicted_interval_95CI_lower", "predicted_interval_95CI_upper", "tau2", "denominator_count", "outcome_count", "number_of_databases"),
    values_to = "estimate_value",
    names_to = "estimate_name"
  ) |>
  mutate(
    ...
    outcome = factor(.data$outcome, levels = c(aesiClean, maeClean)),
    estimate_value = as.character(estimate_value),
    outcome_group = factor(outcome_group, levels = c("Maternal Adverse Events", "Adverse Events of Special Interest"))
  ) |>
  filter(!is.na(outcome)) |>
  ...
  visTable(
    estimateName = c(
      "Databases (N)" = "<number_of_databases>",
      "Pregnancies (N)" = "<denominator_count>",
      "Outcomes (N)" = "<outcome_count>",
      "CI (95% PI)" = "<pooled_estimate> (<predicted_interval_95CI_lower> to <predicted_interval_95CI_upper>)",
      "𝜏²" = "<tau2>"
    ),
    hide = "estimate_type",
    header = "estimate_name",
...
:::{custom-style="Caption"}
Figure 1. Cumulative Incidence curves for Maternal Adverse Events across databases.
:::
...
survivalTidy |>
  ...
  ggh4x::facetted_pos_scales(
    y  = list(
      outcome == "Miscarriage" ~ scale_y_continuous(limits = c(0,0.2), oob = scales::rescale_none),
      outcome == "Ectopic pregnancy" ~ scale_y_continuous(limits = c(0,0.015), oob = scales::rescale_none),
      outcome == "Stillbirth" ~ scale_y_continuous(limits = c(0,0.2), oob = scales::rescale_none),
      outcome == "Antepartum haemorrhage" ~ scale_y_continuous(limits = c(0,0.13), oob = scales::rescale_none),
      outcome == "Gestational diabetes" ~ scale_y_continuous(limits = c(0,0.09), oob = scales::rescale_none),
      outcome == "Pre-eclampsia" ~ scale_y_continuous(limits = c(0,0.1), labels = scales::label_comma(), oob = scales::rescale_none),
      outcome == "Eclampsia"  ~ scale_y_continuous(limits = c(0,0.001), oob = scales::rescale_none),
      outcome == "HELLP Syndrome"  ~ scale_y_continuous(limits = c(0,0.005), oob = scales::rescale_none)
    )
  )
```

:::{custom-style="Caption"}
Figure 2. Cumulative incidence curves for Maternal Adverse Events across databases, stratified by age.
:::
...
pPregnancy <- survivalTidyAge |>
  filter(!grepl("Postpartum", outcome)) |>
  pSurvival() +
  ...
  #   ),
    y  = list(
      outcome == "Ectopic pregnancy" ~ scale_y_continuous(limits = c(0,0.02), oob = scales::rescale_none),
      outcome == "Stillbirth" ~ scale_y_continuous(limits = c(0,0.05), oob = scales::rescale_none),
      outcome == "Antepartum haemorrhage" ~ scale_y_continuous(limits = c(0,0.15), oob = scales::rescale_none),
      outcome == "Gestational diabetes" ~ scale_y_continuous(limits = c(0,0.15), oob = scales::rescale_none),
      outcome == "Pre-eclampsia" ~ scale_y_continuous(limits = c(0,0.1), labels = scales::label_comma(), oob = scales::rescale_none),
      outcome == "Eclampsia" ~ scale_y_continuous(limits = c(0,0.0015), oob = scales::rescale_none),
      outcome == "HELLP Syndrome" ~ scale_y_continuous(limits = c(0,0.009), oob = scales::rescale_none),
      outcome == "Postpartum endometritis" ~ scale_y_continuous(limits = c(0,0.006), oob = scales::rescale_none),
      outcome == "Postpartum haemorrhage" ~ scale_y_continuous(limits = c(0,0.020), oob = scales::rescale_none)
    )
  ) +
  ggtitle("A) Cumulative Incidence curves for Pregnancy Maternal Adverse Events") +
  theme(
    legend.position = "right", 
    panel.spacing.y = unit(1, "lines"), 
    panel.spacing.x = unit(0.7, "lines"),
    plot.title = element_text(hjust = -0.05, face = "bold", size = 13)
  )
pPostpartum <- survivalTidyAge |>
  filter(grepl("Postpartum", outcome)) |>
  pSurvival() +
  facet_grid(rows = vars(outcome), cols = vars(maternal_age_group), scales = "free", labeller = label_wrap_gen(10)) +
  theme(legend.position = "right") +
  ggtitle("B) Cumulative Incidence curves for Postpartum Maternal Adverse Events") +
  theme(
    panel.spacing.y = unit(1, "lines"), 
    panel.spacing.x = unit(0.7, "lines"),
    plot.title = element_text(hjust = -0.05, face = "bold", size = 13)
...
pPregnancy / pPostpartum +
  plot_layout(heights = c(10, 1.9), guides = "collect")
```

:::{custom-style="Caption"}
Figure 3. Incidence rates per 100,000 person-years for the 5-most frequent Adverse Events of Special Interest among pregnant women, stratified by gestational trimester and database.
:::
...
pTrimester <- tidyIncidence |> 
  ...
  pIncidence("gestational_trimester", "free_y") +
...
pAge <- tidyIncidence |> 
  ...
  pIncidence("maternal_age_group", "free_y") +
...
pCalendar <- tidyIncidence |> 
  filter(gestational_trimester == "overall", maternal_age_group == "overall") |> 
  mutate(
    pregnancy_start_period = factor(
      pregnancy_start_period, 
      levels = c("Pre COVID-19", "COVID-19 main outbreak", "Post COVID-19 main outbreak"), 
      labels = c("2018-2019", "2020-2021", "≥2022")
    )
  ) |>
  pIncidence("pregnancy_start_period", "free_y") +
  # scale_y_continuous(limits = c(0,150)) +
  theme(legend.position = "bottom") +
     ggh4x::facetted_pos_scales(
     y  = list(
       outcome_cohort_name == "Deep vein thrombosis" ~ scale_y_continuous(limits = c(0,150), oob = scales::rescale_none),
       outcome_cohort_name == "Pulmonary embolism" ~ scale_y_continuous(limits = c(0,270), oob = scales::rescale_none),
       outcome_cohort_name == "Bell’s palsy" ~ scale_y_continuous(limits = c(0,100), oob = scales::rescale_none),
       outcome_cohort_name == "Immune thrombocytopenia" ~ scale_y_continuous(limits = c(0,100), oob = scales::rescale_none),
       outcome_cohort_name == "Anaphylaxis" ~ scale_y_continuous(limits = c(0,60), oob = scales::rescale_none),
       outcome_cohort_name == "Myocarditis or pericarditis" ~ scale_y_continuous(limits = c(0,25), oob = scales::rescale_none)
     )
   ) +
  ggtitle("C) Incidence Rates by Calendar Period") +
  theme(plot.title = element_text(hjust = -0.05, face = "bold", size = 15)) 

(pTrimester / pAge / pCalendar) + 
  plot_layout(heights = c(1.9, 1.9, 2)) 
```