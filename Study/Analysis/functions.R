...
getLargeScaleCharacteristics <- function(cdm, strata, weights) {
  ...
  if (!weights) {
    ...
    data <- cdm$features_weighted |> mutate(overall = "overall")
    for (strata.k in strata) { # strata name
      strataLevels <- unique(data |> pull(strata.k))
      for(strataLevel.k in strataLevels) { # strata level
        # data
...
cohortExit <- function(x, strata, weights) {
  ...
  weighting <- "FALSE"
  if (weights != "weights_null") weighting <- "TRUE"
...
pivotSurvivalData <- function(data, outcomes) {
  ...
  if ("tbl_sql" %in% class(data)) {
    data |>
      ...
      mutate(
        variable = sql("CASE WHEN CHARINDEX('_time', outcome_name) > 0 THEN 'time' ELSE 'status' END"),
        outcome_name = sql("REPLACE(REPLACE(outcome_name, '_time', ''), '_status', '')")
      ) |>
...
getRiskEstimate <- function(data, group, strata, outcomes, weights = NULL, ci = "midp") {
  ...
  data <- data |>
    ...
    compute(name = name, temporary = FALSE, overview = TRUE)
  
  results <- list()
  
  for (nm in unique(data |> pull(cohort_name))) {
...
summariseGestationalWeek <- function(x, strata.k, weight) {
...
summariseCalendarWeek <- function(x, strata.k, weight) {
  if (strata.k == "overall") strata.k <- NULL
  x |>
...
summariseTimeDistribution <- function(cdm, strata, weights = FALSE) {
  if (!weights) {
    weighting <- "FALSE"
    weightCol <- "weights_null"
    tab <- cdm$study_population |>
      mutate(weights_null = 1)
  } else {
    weighting <- "TRUE"
    tab <- cdm$study_population_weighted
  }
