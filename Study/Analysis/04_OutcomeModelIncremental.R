...
for (end in endDates) {
  ...
  estimateSurvivalRisk(
    ...
    exportSummarisedResult(fileName = paste0("outcome_", end, "_", cdmName(cdm), ".csv"), path = output_folder)
  jj <- jj + 1
}
...
for (endDate in c("week_19_end", "week_19_end_sensitivity")) {
  estimateSurvivalRisk(
    ...
    suppressRiskEstimates() |> 
    exportSummarisedResult(fileName = paste0("outcome_", endDate, "_", cdmName(cdm), ".csv"), path = output_folder)
  jj <- 1 + jj
}
...
# NCO ----
info(logger, "- Negative Control Outcomes Table")
...
if (getNCO) {
  ...
  info(logger, "- Negative Control Outcomes Sensitivity")
  estimateSurvivalRisk(
    cohort = cdm$study_population_nco, outcomes = settings(cdm$nco)$cohort_name, 
    ...
    suppressRiskEstimates() |> 
    exportSummarisedResult(fileName = paste0("nco_sensitivity_", cdmName(cdm), ".csv"), path = output_folder)
  
  info(logger, "- Positive Control Outcomes")
  estimateSurvivalRisk(
    ...
    suppressRiskEstimates() |> 
    exportSummarisedResult(fileName = paste0("pco_", cdmName(cdm), ".csv"), path = output_folder)
  
  info(logger, "- Positive Control Outcomes Sensitivity")
  estimateSurvivalRisk(
    cohort = cdm$study_population_pco, outcomes = "covid", 
    end = "cohort_end_date_sensitivity", strata = strata, group = "cohort_name", 
    weights = allCovariatesPS, outcomeGroup = "Positive Control Outcomes", ci = ci
    ...
    suppressRiskEstimates() |> 
    exportSummarisedResult(fileName = paste0("pco_sensitivity_", cdmName(cdm), ".csv"), path = output_folder)
} 
